...
/*   Updated: 2023/11/27 15:18:48 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	line = build_line(buffer_chunk_control.first_node, remain_buffer[fd],
			buffer_chunk_control.node_counter, newline.index, bytes_read);
	free_t_buffer_chunk(buffer_chunk_control.first_node);
	return (line);
}
...
char	*build_line(t_buffer_chunk *first, char *remain_buffer,
		unsigned int node_counter, size_t newline_index, size_t bytes_read)
{
	char	*line;
	size_t	i;
	size_t	j;
	...
	line = malloc(sizeof(char) * ((bytes_read * (node_counter - 1)
					+ newline_index + 2) + gnl_strlen(remain_buffer)));
	if (!line)
		return (line);
