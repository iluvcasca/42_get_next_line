...
/*   Updated: 2023/11/27 16:11:07 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char				remain_of_buffer[FD_SIZE][BUFFER_SIZE];
	int						bytes_read;
	t_buffer_control	buffer_control;
	t_newline				newline;
	...
	init_t_buffer_control(&buffer_control);
	if (!buffer_control.first_node)
		return (NULL);
	buffer_control.node = buffer_control.first_node;
	bytes_read = read(fd, buffer_control.node->buffer, BUFFER_SIZE);
	if (check_failure(buffer_control, bytes_read,
			gnl_strlen(remain_of_buffer[fd])))
		return (NULL);
	while (!is_line(buffer_control.node->buffer, &newline, bytes_read)
		&& bytes_read == BUFFER_SIZE)
	{
		if (check_failure(add_back_t_buffer_chunk(&buffer_control),
				bytes_read, gnl_strlen(remain_of_buffer[fd])))
			return (NULL);
		bytes_read = read(fd, buffer_control.node->buffer, BUFFER_SIZE);
	}
	return (build_line(buffer_control, remain_of_buffer[fd], newline,
			bytes_read));
}

char	*build_line(t_buffer_control buffer_control,
		char *remain_buffer, t_newline newline, int bytes_read)
{
	...
	s1 = ((buffer_control.node_counter - 1) * BUFFER_SIZE + newline.index
			+ 2 + gnl_strlen(remain_buffer));
	s2 = ((buffer_control.node_counter - 1) * BUFFER_SIZE + bytes_read + 1
	...
	while (buffer_control.node_counter > 1)
	{
		i += gnl_strlcpy(line + i, buffer_control.node->buffer,
				BUFFER_SIZE);
		buffer_control.node = buffer_control.node->next;
		buffer_control.node_counter--;
	}
	if (newline.is_found)
	{
		i += gnl_strlcpy(line + i, buffer_control.node->buffer,
				newline.index + 1);
		gnl_strlcpy(remain_buffer, buffer_control.node->buffer
	...
	line[i] = '\0';
	free_t_buffer_chunk(buffer_control.first_node);
	return (line);
}
...
unsigned int	check_failure(t_buffer_control buffer_control,
...
t_buffer_control	add_back_t_buffer_chunk(t_buffer_control *buffer_control)
{
	...
	buffer_control->node->next = last;
	buffer_control->node = last;
	buffer_control->node_counter++;
	if (!last)
...
void	init_t_buffer_control(t_buffer_control *buffer_control)
{
	...
	buffer_control->first_node = first_node;
	buffer_control->node_counter = 1;
}
